    ...
    <div class="admin-page">
      ...
      <div class="row items-center q-mb-md">
        ...
        <div class="col-auto">
          <q-btn
            color="primary"
            :label="$t('admin.actions.add') + ' ' + $t('admin.models.director')"
            icon="add"
            :to="{ name: 'AdminDirectorAdd' }"
          />
        </div>
      </div>
...
const { t } = useI18n()
const { loading, error, fetchDirectors, deleteDirector } = useAdminDirectors()
...
const viewDetail = (id: number) => {
  router.push(`/management/directors/${id}`)
}

const editItem = (id: number) => {
  router.push(`/management/directors/${id}/change`)
}
...
const loadDirectors = async () => {
  try {
    ...
    const response = await fetchDirectors(params)
