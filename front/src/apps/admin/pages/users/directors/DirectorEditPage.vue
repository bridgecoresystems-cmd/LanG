  ...
  <AdminLayout>
    <div class="admin-page">
      <div class="page-header">
        <h1>{{ $t('admin.actions.edit') }} {{ $t('admin.models.director') }}</h1>
        <router-link to="/management/directors" class="btn btn-secondary">
          <i class="pi pi-arrow-left"></i>
          {{ $t('common.back') }}
        </router-link>
      </div>

      <div v-if="loading && !director" class="loading-state">
...
const { t } = useI18n()
const { loading, error, fetchDirector, updateDirector } = useAdminDirectors()
...
const loadDirector = async () => {
  try {
    ...
    const data = await fetchDirector(id)
    director.value = data
    formData.value = {
...
const handleSubmit = async () => {
  ...
  try {
    const hasFiles = formData.value.avatar instanceof File
    ...
    await updateDirector(parseInt(route.params.id as string), dataToSend)
    ...
    if (err.response?.data) {
      ...
      if (typeof errorData === 'object') {
        ...
        error.value = errorMessages.join('; ') || 'Failed to update director'
      } else {
        error.value = errorData || 'Failed to update director'
      }
    } else {
      error.value = err.message || 'Failed to update director'
    }
  }
}
