    ...
    <div class="admin-page">
      ...
      <div class="row items-center q-mb-md">
        ...
        <div class="col-auto">
          <q-btn
            color="primary"
            :label="$t('admin.actions.add') + ' ' + $t('admin.models.headTeacher')"
            icon="add"
            :to="{ name: 'AdminHeadTeacherAdd' }"
          />
        </div>
      </div>
...
const { t } = useI18n()
const { loading, error, fetchHeadTeachers, deleteHeadTeacher } = useAdminHeadTeachers()
...
const viewDetail = (id: number) => {
  router.push(`/management/head-teachers/${id}`)
}

const editItem = (id: number) => {
  router.push(`/management/head-teachers/${id}/change`)
}
...
const handleDelete = async (id: number) => {
  ...
  if (confirmed) {
    try {
      await deleteHeadTeacher(id)
      await loadHeadTeachers()
    } catch (err) {
...
const loadHeadTeachers = async () => {
  try {
    ...
    const response = await fetchHeadTeachers(params)
    items.value = Array.isArray(response) ? response : response.results || []
  } catch (err) {
    console.error('Load head teachers error:', err)
  }
}
