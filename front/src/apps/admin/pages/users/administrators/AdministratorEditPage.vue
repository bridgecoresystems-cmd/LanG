  ...
  <AdminLayout>
    <div class="admin-page">
      <div class="page-header">
        <h1>{{ $t('admin.actions.edit') }} {{ $t('admin.models.administrator') }}</h1>
        <router-link to="/management/administrators" class="btn btn-secondary">
          <i class="pi pi-arrow-left"></i>
          {{ $t('common.back') }}
        </router-link>
      </div>

      <div v-if="loading && !administrator" class="loading-state">
...
const { t } = useI18n()
const { loading, error, fetchAdministrator, updateAdministrator } = useAdminAdministrators()

const administrator = ref<any>(null)
const formData = ref({
  username: '',
  password: '',
...
const loadAdministrator = async () => {
  try {
    ...
    const data = await fetchAdministrator(id)
    ...
    console.error('Load administrator error:', err)
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await updateAdministrator(parseInt(route.params.id as string), dataToSend)
    ...
    router.push('/management/administrators')
  } catch (err: any) {
    ...
    if (err.response?.data) {
      ...
      if (typeof errorData === 'object') {
        ...
        error.value = errorMessages.join('; ') || 'Failed to update administrator'
      } else {
        error.value = errorData || 'Failed to update administrator'
      }
    } else {
      error.value = err.message || 'Failed to update administrator'
    }
  }
}
