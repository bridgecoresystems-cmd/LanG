    ...
    <n-space vertical size="large" class="head-teacher-page">
      <div class="page-header">
        <n-space align="center" size="large">
          ...
          <div>
            <n-h1 style="margin: 0;">{{ $t('admin.actions.edit') }} {{ $t('admin.models.courses') }}</n-h1>
            <n-text depth="3">Редактирование учебного курса</n-text>
          </div>
        </n-space>
      </div>
...
const { t } = useI18n()
const { loading, error, fetchCourse, updateCourse } = useHeadTeacherCourses()
...
const handleSubmit = async () => {
  ...
  try {
    ...
    logActivity(
      'update',
      `Updated course "${dataToSend.name}"`,
      'Course',
      id,
      dataToSend.name
    )
...
onMounted(() => {
  logActivity('view', 'View course edit page')
  loadCourse()
})
</script>
