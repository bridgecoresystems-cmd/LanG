...
// Import Read-only Tab Components
import AttendanceReadOnlyTab from './tabs/AttendanceReadOnlyTab.vue'
import GradesReadOnlyTab from './tabs/GradesReadOnlyTab.vue'
import StudentsReadOnlyTab from './tabs/StudentsReadOnlyTab.vue'
import LessonsReadOnlyTab from './tabs/LessonsReadOnlyTab.vue'
import GamesReadOnlyTab from './tabs/GamesReadOnlyTab.vue'
...
const activeTabComponent = computed(() => {
  switch (activeTab.value) {
    case 'attendance': return AttendanceReadOnlyTab
    case 'grades': return GradesReadOnlyTab
    case 'students': return StudentsReadOnlyTab
    case 'lessons': return LessonsReadOnlyTab
    case 'games': return GamesReadOnlyTab
    default: return AttendanceReadOnlyTab
  }
})

const goBack = () => {
  // Go back to groups list based on user role
  const userRole = route.path.includes('/head-teacher/') ? 'head-teacher' : 'director'
  router.push(`/cabinet/${userRole}/groups`)
}
...
const loadGroupInfo = async () => {
  ...
  error.value = null
  try {
    const groupRes = await api.get(`/courses/groups/${groupId}/`)
