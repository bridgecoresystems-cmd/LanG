...
- Проверь что backend запущен: `bun run dev`

---

## Запуск сервера (доступ с телефона и терминала)

Чтобы заходить с телефона и подключать ESP32 терминал по IP твоего ПК:
...
Backend и frontend слушают на **0.0.0.0** — доступны в локальной сети.
...
**2. Телефон:** в браузере открой `http://192.168.1.100:3000`

**3. Терминал:** в `terminal.ino` укажи:
```cpp
#define SERVER_HOST "http://192.168.1.100:8010"
```

ПК, телефон и ESP32 должны быть в одной WiFi сети.
