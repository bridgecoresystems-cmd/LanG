  ...
  <div class="cabinet-page">
    <header class="page-header">
      <div class="page-header__text">
        ...
        <p class="page-header__subtitle">История всех транзакций. Редактирование доступно главбуху и бухгалтеру.</p>
      </div>
      <div class="page-header__actions">
        <NSpace>
          ...
          <NButton type="primary" size="large" @click="navigateTo('/admin/head-accountant/payments/add')">
    ...
    <NCard class="cabinet-card search-card">
      <NGrid :cols="24" :x-gap="12" :y-gap="12" item-responsive responsive="screen">
        ...
        <NGi span="24 m:2 l:2">
          <NButton type="primary" size="large" block @click="loadPayments">
            <template #icon>
...
// ACCOUNTANT тоже может редактировать (как и HEAD_ACCOUNTANT, GEN_DIRECTOR)
const canEdit = computed(() => ['HEAD_ACCOUNTANT', 'GEN_DIRECTOR', 'ACCOUNTANT', 'SUPERUSER'].includes(authStore.user?.role || ''))
const isHeadAccountant = computed(() => canEdit.value)
...
const { pagination, resetPage } = useCabinetPagination('admin-payments-report')

watch(() => pagination.page, loadPayments)
watch(() => pagination.pageSize, () => { pagination.page = 1; loadPayments() })

const columns = computed(() => {
  const cols: any[] = [
    { title: 'ID', key: 'id', width: 70 },
    { title: 'Дата', key: 'created_at', render: (row: any) => new Date(row.created_at).toLocaleDateString('ru-RU', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) },
    ...
    { title: 'Назначение', key: 'purpose' },
    {
      ...
      render: (row: any) => {
        ...
        return h('div', [
          h(NText, { type: 'success', strong: true }, { default: () => `${row.total} TMT` }),
          discount > 0 ? h('div', { style: 'font-size: 11px; color: #f0a020' }, { default: () => `Скидка: ${discount} TMT` }) : null,
        ])
      },
    },
  ]
  ...
  cols.push({
    title: 'Действия', key: 'actions', align: 'right',
    render: (row: any) => {
      const actions = [
        h(NTooltip, { trigger: 'hover' }, {
          trigger: () => h(NButton, {
            size: 'small', quaternary: true, circle: true,
            onClick: () => navigateTo(`/admin/head-accountant/payments/${row.id}/receipt`),
      ...
      if (canEdit.value) {
        actions.push(
          h(NTooltip, { trigger: 'hover' }, {
            trigger: () => h(NButton, {
              size: 'small', quaternary: true, circle: true, type: 'info',
              onClick: () => navigateTo(`/admin/head-accountant/payments/${row.id}/edit`),
...
const loadPayments = async () => {
  ...
  try {
    const { data } = await api.api.v1.cabinet.accountant.payments.get({
      query: {
        limit: String(pagination.pageSize ?? 50),
        offset: String(((pagination.page || 1) - 1) * (pagination.pageSize ?? 50)),
...
const exportToExcel = () => {
  ...
  const data = payments.value.map(p => ({
    'ID': p.id, 'Дата': new Date(p.created_at).toLocaleString('ru-RU'), 'Плательщик': p.payer,
    'Группа': p.group_name || '—', 'Назначение': p.purpose, 'Сумма (TMT)': p.total,
    'Метод': p.method === 'cash' ? 'Наличные' : p.method === 'card' ? 'Карта' : 'Банк',
    'Школа': p.school_name || '—', 'Создал': p.created_by || '—',
  }))
...
watch([searchQuery, methodFilter], () => { pagination.page = 1; loadPayments() })
...
.search-card { margin-bottom: 24px; padding: 8px; }
.filter-label { font-size: 12px; color: var(--n-text-color-3); margin-top: 4px; }
.table-card { overflow: hidden; }
:deep(.cabinet-data-table) { --n-border-radius: 16px; }
:deep(.n-data-table-th) { background-color: transparent !important; font-weight: 600 !important; color: var(--n-text-color-3) !important; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.05em; padding: 16px 24px !important; }
:deep(.n-data-table-td) { padding: 16px 24px !important; font-size: 0.9375rem; }
:deep(.n-data-table-tr:hover .n-data-table-td) { background-color: #f9fafb !important; }
</style>
