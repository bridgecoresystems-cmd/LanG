      ...
      <template #header>
        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
          ...
          <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
            ...
            />
            <NButton quaternary size="small" @click="navigateTo('/admin/gems/requests')">
...
const ROLE_LABELS: Record<string, string> = {
  HEAD_ACCOUNTANT: 'Гл. бухгалтер', ACCOUNTANT: 'Бухгалтер', TEACHER: 'Учитель', STUDENT: 'Ученик',
  MERCHANT: 'Вендор', SUPERUSER: 'Суперадмин', GEN_DIRECTOR: 'Ген. директор',
}

const totalBalance = computed(() => wallets.value.reduce((s, w) => s + w.balance, 0))
const filteredWallets = computed(() => {
  const q = search.value.toLowerCase().trim()
  return q ? wallets.value.filter((w) => w.name.toLowerCase().includes(q)) : wallets.value
})

const columns = computed(() => [
  { title: 'Имя', key: 'name', render: (row: any) => h('span', { style: 'font-weight: 600;' }, row.name || row.userId) },
  { title: 'Школа', key: 'school_name', width: 180, ellipsis: { tooltip: true }, render: (row: any) => row.school_name || '—' },
  {
    title: 'Роль', key: 'role', width: 140,
    render: (row: any) => {
      const r = (row.role || '').toUpperCase()
      const typeMap: Record<string, any> = { STUDENT: 'info', TEACHER: 'success', ACCOUNTANT: 'warning', MERCHANT: 'default' }
      return h(NTag, { type: typeMap[r] || 'default', size: 'small', round: true }, { default: () => ROLE_LABELS[r] || r })
    },
  },
  {
    title: 'Баланс 💎', key: 'balance', width: 130, sorter: (a: any, b: any) => a.balance - b.balance,
    render: (row: any) => h('span', { style: `font-weight: 700; font-size: 15px; color: ${row.balance > 0 ? '#15803d' : '#9ca3af'}` }, `${row.balance} 💎`),
  },
  {
    title: '', key: 'actions', width: 110,
    render: (row: any) => {
      if (!['ACCOUNTANT', 'TEACHER', 'STUDENT'].includes((row.role || '').toUpperCase())) return null
      return h(NButton, { size: 'small', type: 'primary', ghost: true, onClick: () => openSend(row) }, { default: () => 'Начислить' })
    },
  },
])
...
async function doSend() {
  ...
  try {
    const res = await $fetch<any>(`${API}/cabinet/gems/transfer`, {
      ...
      body: { receiverId: selectedUser.value.userId, amount: sendAmount.value, comment: sendComment.value || undefined },
    })
...
.stats-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 0; }
.stat-card { border-radius: 14px; background: linear-gradient(135deg, #f0fdf4, #dcfce7); }
.stat-card__label { font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
.stat-card__value { font-size: 28px; font-weight: 900; color: #111827; }
