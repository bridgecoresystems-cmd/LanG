<template>
  <div class="grades-page">
    ...
    <!-- Loading -->
    <div v-if="loading" class="grades-loading">
      <NSpin size="large" />
    </div>
    ...
    <div v-else-if="!data?.scheme" class="empty-state">
      ...
      <p>Как только завуч настроит экзамены для вашей группы, результаты появятся здесь</p>
    </div>
...
import { NCard, NTag, NProgress, NIcon, NSpin, useMessage } from 'naive-ui'
import {
...
async function loadData() {
  loading.value = true
  try {
    const result = await $fetch<any>(`${API}/cabinet/student/groups/${groupId.value}/exam-results`, {
      credentials: 'include',
    })
    data.value = result
  } catch (e) {
    console.error('Exam results load failed', e)
  } finally {
    loading.value = false
  }
}
...
<style scoped>
.grades-page { padding-bottom: 60px; }
