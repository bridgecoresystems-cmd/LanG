<template>
  <div class="cabinet-page-content">
    <div class="row items-center justify-between q-mb-md">
      <h1 class="text-h4 q-ma-none">Категории курсов (Редактор)</h1>
      <q-btn color="primary" icon="add" label="Добавить категорию" to="/cabinet/editor/categories/add" />
    </div>
    ...
    <q-card flat bordered>
      <q-card-section>
        <q-table
          ...
          bordered
          :loading="loading"
          @row-click="(_evt, row) => router.push(`/cabinet/editor/categories/${row.id}`)"
        ...
        >
          ...
          <template v-slot:body-cell-actions="props">
            <q-td :props="props" @click.stop class="q-gutter-xs">
              <q-btn flat round color="primary" icon="edit" @click="router.push(`/cabinet/editor/categories/${props.row.id}`)" />
...
const columns = [
  { name: 'image', label: 'Фото', field: 'image', align: 'left' as const },
  { name: 'name_ru', label: 'Название (RU)', field: 'name_ru', align: 'left' as const, sortable: true },
  { name: 'order', label: 'Порядок', field: 'order', align: 'center' as const, sortable: true },
  { name: 'is_active', label: 'Статус', field: 'is_active', align: 'center' as const, format: (val: boolean) => val ? 'Активна' : 'Неактивна' },
